...
export function AuthorsCard({ name, description, socialLinks, URL, image }: {
    name: string;
    description: string;
  ...
  return (
    <Card className="m-3 items-center flex flex-col rounded-lg shadow sm:flex">
      {image !== undefined
        ? (
          <CardHeader className="flex flex-row items-center justify-between">
            <Link href={URL}>
              <Image
                src={imageUrl}
                alt={image.alt !== undefined ? image?.alt : name}
                width={"1224"}
                height={"724"}
